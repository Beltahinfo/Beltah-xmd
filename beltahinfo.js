...
// NOTE: Updated store initialization to avoid crash when makeInMemoryStore is not exported
// Some versions of @whiskeysockets/baileys do not export makeInMemoryStore the same way.
// Provide a safe fallback minimal in-memory store to prevent the bot from exiting with TypeError.

let store;
try {
  if (typeof baileys_1.makeInMemoryStore === 'function') {
    store = baileys_1.makeInMemoryStore({
      logger: pino().child({
        level: "silent",
        stream: "store"
      })
    ...
    // fallback store
    console.warn("makeInMemoryStore is not available on the baileys import. Using fallback in-memory store.");
    store = {
      chats: {},
      contacts: {},
      bind: () => { /* noop for fallback */ },
      writeToFile: () => { /* noop for fallback */ },
      async loadMessage(remoteJid, id) {
        const chat = this.chats[remoteJid] || [];
        return chat.find(m => m.key?.id === id) || undefined;
      }
    };
  }
} catch (err) {
  console.warn("Error initializing makeInMemoryStore, falling back to minimal store:", err);
  store = {
    chats: {},
    contacts: {},
    bind: () => { /* noop */ },
...
const getContextInfo1 = (title = '', userJid = '', thumbnailUrl = '', conf = {}) => ({
  ...
  externalAdReply: {
    ...
    zk.ev.on("messages.upsert", async m => {
      ...
      /**
       * Helper: extract text or caption or selection id from a received message.
       * This replaces a long, error-prone ternary chain and avoids syntax errors.
       */
      function extractTextFromMessage(ms, mtype) {
        try {
          ...
          if (mtype === "templateButtonReplyMessage") return ms.message.templateButtonReplyMessage?.selectedId || '';
          if (mtype === "buttonsResponseMessage") return ms.message.buttonsResponseMessage?.selectedButtonId || '';
          // fallback: try a few safe fields
      ...
      var texte = extractTextFromMessage(ms, mtype);
  ...
  try {
    ...
    console.error(e);
              ...
              try {
                ...
                if (ms.message[mtype].contextInfo.mentionedJid && (ms.message[mtype].contextInfo.mentionedJid.includes(idBot) ||  ms.message[mtype].contextInfo.mentionedJid.includes(conf.NUMERO_OWNER [...]
