...
/**
 * Construct WhatsApp contextInfo for external ad reply.
 */
...
/**
 * Search for a Spotify track using the API.
 */
...
/**
 * Search for a SoundCloud track using the API.
 */
...
/**
 * Search for a YouTube video using yt-search.
 */
...
/**
 * Download a Spotify track using an external API.
 */
...
/**
 * Download a SoundCloud track using an external API.
 */
...
/**
 * Download a YouTube audio track using an external API.
 */
...
keith(
  ...
  async (dest, zk, commandOptions) => {
    ...
    if (!arg[0]) {
      return repondre(zk, dest, ms, "Please provide a song name or URL.");
    }
    ...
    // Detect platform based on query or search all
    const platforms = [];
    ...
    // Try each platform in order until success
    for (const platform of platforms) {
      try {
        ...
        // Use 'track.url' if available, else fallback to 'query'
        const trackUrl = track.url || track.link || query;
        ...
        downloadData = await downloadFn(trackUrl);
    ...
    const fileName =
      `${track.title} - ${artist}.${downloadData.format}`.replace(/[^\w\s.-]/gi, '');
