...
keith(
    ...
    async (message, client, config) => {
        ...
        // Check for argument to display a specific category
        ...
        if (chosenCategoryIndex === null) {
            // Display numbered menu of categories
            let menuMessage = `Hello 👋   *${nomAuteurMessage || "User"}*\n\n`;
            menuMessage += `╭───「 *${settings.BOT}* 」───╮ 
...
${readMore}
🛡️ *COMMAND CATEGORIES MENU* 🛡️\n`;

            sortedCategories.forEach((cat, i) => {
                menuMessage += `*${i + 1}.* ${toFancyUppercaseFont(cat)}\n`;
            });

            menuMessage += `\n_Reply or send "${prefix}menu <number>" to see commands in a category._\n`;
